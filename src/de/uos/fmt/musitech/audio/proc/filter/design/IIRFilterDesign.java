...
/**
 * IIRFilterDesign does the actual filter designing.
 */
...
//      by P. Lynn and W. Fuerst (Prentice Hall)
    ...
    public float[] getACoeffs() {
        float floatAC[] = new float[aCoeff.length];
        for (int i = 0; i < floatAC.length; i++) {
            floatAC[i] = (float)aCoeff[i];
        }
        return floatAC;
    }
    ...
    public float[] getBCoeffs() {
        float floatBC[] = new float[bCoeff.length];
        for (int i = 0; i < floatBC.length; i++) {
            floatBC[i] = (float)bCoeff[i];
        }
        return floatBC;
    }
	...
	/**
     * Designs a filter with normalized response, i.e. the maximum response is 1
     * (at the frequency points).
     * 
     * @param freqPoints The number of points, where to check the filter response.
     *            response.
     */
	public void designNormalized(int freqPoints){
	    design();
