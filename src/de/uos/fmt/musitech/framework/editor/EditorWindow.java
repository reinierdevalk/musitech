...
public class EditorWindow extends JFrame implements ActionListener {
	...
	/**
	 * Constructs a new EditorWindow with the given string as titel.
	 * <br>
	 ...
	 * <br>
	 * The EditorWindow is given a FocusListener which calls <code>focusReceived()</code>
	 * of the editor contained in the EditorPanel. This is done in order to update the
	 * editor's GUI when this EditorWindow comes to the foreground.
	 * 
	 * @param title String to be displayed on top of the EditorWindow
	 */
	...
	/**
	 * Calls method <code>focusReceived()</code> of the editor contained in the 
	 * EditorPanel.
	 */
	private void focusEditor(){
	    if (editorPanel.getEditor()!=null && editorPanel.getEditor().size()>0){
	        Collection editors = editorPanel.getEditor();
	        for (Iterator iter = editors.iterator(); iter.hasNext();) {
                Editor element = (Editor) iter.next();
                element.focusReceived();
            }
	    }
	}
}
