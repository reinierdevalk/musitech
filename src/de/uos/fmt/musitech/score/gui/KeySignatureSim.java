...
public class KeySignatureSim implements SpringAttacher
{
   private Vector keySignatures = new Vector();
   private int optimalSpace = 0;
   private int rwidth = 0;
   
   public void add (KeySignature keySignature)
   {
 		keySignatures.add(keySignature);	     
 		optimalSpace = Math.max(optimalSpace, keySignature.optimalSpace());
   ...
   public void setXPos(int x)
   {
      ...
      {
         KeySignature keySignature = (KeySignature)keySignatures.get(i);
      	keySignature.setXPos(x);
      }
   }
   ...
   public void setParentXPos(int x)
   {
      ...
      {
         KeySignature keySignature = (KeySignature) keySignatures.get(i);
	      if (keySignature.getScoreParent() != null)
      		keySignature.getScoreParent().setXPos(x);
      }
   }
   ...
   public void setMeasureRWidth(int width)
   {
      ...
      {
         KeySignature keySignature = (KeySignature)keySignatures.get(i);
         Measure m = keySignature.measure();
	      if (m != null)
      		m.setRWidth(width);
      }      
   }
