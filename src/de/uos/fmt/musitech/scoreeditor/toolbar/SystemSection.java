 ...
 * @version 30.08.2007
 */
public class SystemSection extends AbstractToolbarSection {
	
	private JComboBox systembox;
	
	public SystemSection(SectionController tbp) {
		super(tbp);
	}
	...
	protected void createItems() {
		...
		for (int i = 0; i < getScoreEditor().getNotationSystem().size(); i++) {
	...
	public void update() {
		if (systembox.getItemCount() != getScoreEditor().getNotationSystem().size()) {
			...
			for (int i = 0; i < getScoreEditor().getNotationSystem().size(); i++) {
				systembox.addItem("System" + (i+1)); //$NON-NLS-1$
			}
			systembox.setSelectedIndex(getScoreEditor().getSelectedStaff());
		}
	}
