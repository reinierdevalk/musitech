...
public class ExampleObjects {
	...
	public static MusicCollection fromMIDI() {
		MusicCollection workCollection = new MusicCollection();
		workCollection.setName("Test Collection");

		try {
			Piece work2 =
				new MidiReader().getPiece(
					new File("de/uos/fmt/musitech/gin/DadmeAlbricias.mid").toURL());
	...
	public static Piece getWork() {
		Piece work = null;
		try{
			work = new MidiReader().getPiece(new File("../musitech/src/de/uos/fmt/musitech/bach.mid").toURL());
		}catch (Exception e) {
			e.printStackTrace();
		} // end try/catch
		return work;
