...
import de.uos.fmt.musitech.mpeg.FileViewer;
 ...
 */
public class WriteSelectionExample {

    public static void main(String[] args) {
        Piece piece = FileViewer.createTestPiece();
        piece.generateScore();
        
        Document doc = MusiteXMLSerializer.newMPEGSerializer().serialize(piece);
        String xml = XMLHelper.asXML(doc);
        try {
            FileWriter fw = new FileWriter("tmp/selectionExample.xml");
            fw.write(xml);
            fw.close();
        } catch (Exception e) {
            e.printStackTrace();
