 ...
 */
public abstract class TestCase extends JFrame {
	ScoreMapper mapper;
	protected NotationSystem system;
	public Context context = new Context(new Piece());
	...
	public TestCase(String filename) {
		...
		if (filename != null) {
			...
			context.getList().remove(context.getPiece());
			context.getList().add((Piece) MusiteXMLSerializer.newMPEGSerializer().deserialize(doc));
			*/
		}
	...
	void initGUI() {
		...
		String s = this.getClass().getName().toLowerCase();
		s = s.substring(s.lastIndexOf('.') + 1);
		try {
			URL url = this.getClass().getResource(s + ".jpg");
			if(url!=null) {
			ImageIcon ii = new ImageIcon(url);
			JLabel l = new JLabel(ii);
			double scale = Math.min(800.0 / l.getSize().width,
					                600.0 / l.getSize().height);
			l.setSize((int)(l.getSize().width * scale),
					  (int)(l.getSize().height * scale));
