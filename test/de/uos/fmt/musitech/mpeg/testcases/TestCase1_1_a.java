 ...
 *
 */
public class TestCase1_1_a extends TestCase {
...
//		cont.add(barlineContainer);
	...
	void oboeVoice2(Container cont) {
		...
		for (Iterator iter = getNotes(notes, 6, 8).iterator(); iter.hasNext();) {
		...
		for (Iterator iter = getNotes(notes, 9, 14).iterator(); iter.hasNext();) {
			Note element = (Note) iter.next();
			element.setRenderingHints(rh);
		}
		
		//Measure 5:
		for (Iterator iter = getNotes(notes, 15, 16).iterator(); iter.hasNext();) {
			Note element = (Note) iter.next();
			element.setRenderingHints(rh);
		}
	...
	void clarinetVoice2(Container cont) {
		...
		RenderingHints rh = new RenderingHints();
		rh.registerHint("duplicate of other voice", new Boolean(true));
		for (Iterator iter = getNotes(notes, 7, 8).iterator(); iter.hasNext();) {
			Note element = (Note) iter.next();
			element.setRenderingHints(rh);
		}
		MetricAttachable ma = new MetricAttachable(new Rational(7, 4), new StringSymbol("a 2"));
...
//		barlineContainer.add(new Barline(notes.getMetricDuration()));
	...
	public TestCase1_1_a(String filename) {
		super(filename);
	}
	
	public TestCase1_1_a() {
		super();
	}

	public static void main(String argv[]) {
		(new TestCase1_1_a("tmp/testcase1.xml")).setVisible(true);
		//(new TestCase1_1_a()).setVisible(true);
	}
}
