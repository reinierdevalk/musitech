...
/**
 * This class is just a little test for the KernReader. 
 * The file kern.g should be changed such that it can parse these files. Ultimately, the annotations 
 * should be added as well.
 * 
 * @author Tillman Weyde
 */
public class KernReaderTest extends JFrame {
	
    /**
	 * constructor. 
	 * @param filename the file to process
	 */
	public KernReaderTest(String filename) {
		super();
		...
		try {
		    ...
		    notationSystem = KernReader.getNotesFromKern(url);
		} catch (Exception e){
			throw new Error(e);
		}
		
		NotationStaff nstaff = notationSystem.get(0);
		NotationVoice nvoice = nstaff.get(0);
		for (int i=0; i< nvoice.size(); i++) {
			NotationChord nchord = nvoice.get(i);
			if(nvoice.beginOfSlur(nchord))
				System.out.println("***** New Segment *****");
			Note note = nchord.get(0);
			System.out.println("Note "+i+", Name: "+note.getScoreNote().toString()+", MIDI pitch: " +note.getMidiPitch());
		};
		...
		ScoreMapper scoreMapper = new ScoreMapper(scorePanel, notationSystem);
	...
	public static void main(String[] argv) {
	    ...
	    KernReaderTest kernReaderTestFrame = null;
		...
		try {
			...
			kernReaderTestFrame = 
			    new KernReaderTest(filename);
		    //kernReaderTestFrame.setVisible(true);
